...
const ShareClothes = () => {
    ...
    const [isAddingToShared, setIsAddingToShared] = useState(false);
    ...
    // Load already shared URLs on component mount
    React.useEffect(() => {
        if (!userId) return; // Don't load if user not logged in

        console.log("Loading already shared items for userId:", userId, "groupCode:", groupCode);
        ...
        console.log("Existing clothes from localStorage:", existingClothes);
        const sharedUrls = new Set(existingClothes.map(item => item.url));
        console.log("Already shared URLs:", Array.from(sharedUrls));
        setAlreadySharedUrls(sharedUrls);
    }, [groupCode, userId]);
                    ...
                    <div className="share-clothes-actions">
                        <div className="closet-icon-container">
                            <button
                                ...
                                disabled={selectedItems.size === 0 || isAddingToShared}
                            >
